...
- [[贝尔实验室与计算机通信技术史]]：认识 Bell Labs，串起晶体管、香农信息论、Unix 与 C 语言，区分机构、器件、理论、操作系统和语言。
- [[CMD、Bash与PowerShell]]：终端、Shell 和命令是什么，以及三种常见命令解释器的区别。
- [[Termux：Android上的终端与Linux工具环境]]：理解手机终端、原生程序与远程 SSH 的区别，认识包管理、root 权限、PRoot、安装渠道及 Android 后台限制。
...
- [[边缘计算模块与边缘网关]]：区分本地计算、硬件/软件模块和现场网关，理解协议适配、云边分工、离线缓存、数据外发，以及与路由器和模型网关的边界。
- [[Linux的主要用途、普及原因与桌面现状]]：理解 Linux 在服务器、云、Android 和嵌入式设备中的用途，以及传统 PC 较少预装 Linux 的生态原因。
- [[Ubuntu：Linux发行版、软件管理与架构选择]]：理解 Ubuntu 与 Linux/Debian、Desktop/Server/Core、APT 软件包、LTS，以及 amd64/arm64、WSL/虚拟机和设备兼容。
...
- [[CPU、指令集与计算机体系结构]]：理解取指执行、ISA 与微架构，全面比较 x86/Arm 的指令、性能功耗、软件驱动、虚拟化、扩展与成本，并用 IBM 系列机理解兼容。
...
- [[Java SE、JDK与JVM：Java基础平台|Java SE、JDK 与 JVM]]：区分语言、标准版、开发工具包与虚拟机；认识 Java 应用场景、工业管理与现场控制的分层，以及硬实时边界。
- [[Maven：Java项目构建与依赖管理|Maven]]：理解 pom.xml、依赖坐标、构建阶段与插件，区分获取依赖、打包、发布制品和真正上线运行。
...
- [[SDK与API]]：软件之间如何交流、工具包怎样帮助调用接口；以微信生态解释有条件开放，以及开放接口、开放源码、商户权限、用户授权和网络连通的区别。
- [[SSE与流式响应]]：服务器怎样通过一条 HTTP 连接持续向客户端推送文本和事件。
- [[DNS域名系统]]：域名怎样通过根服务器、递归解析、权威服务器和缓存得到 IP 等记录。
- [[CDN内容分发网络：边缘缓存、回源与加速]]：理解源站、缓存命中、回源与更新策略，区分 DNS、代理、HTTPS 和边缘计算；了解节点建设、网站接入、CDN 服务商官方入口与选型前提，以及共享缓存的隐私边界。
- [[直播平台开发前的需求、架构、成本与验收清单]]：区分媒体分发与业务系统，核对费用与交付；结合私域营养品授课，梳理健康宣传与现金红包的权限、身份、安全、幂等和对账，并明确微信支付官方申请、咨询与开发团队的分工。
...
- [[软件供应链：代码签名、SBOM与发布门禁]]：理解包名仿冒、依赖混淆、恶意脚本，以及锁文件、哈希、私有源和扫描的防护边界；结合签名、组件清单和发布门禁追溯制品。
...
- [[渲染逻辑]]：数据如何变成界面；用“先上菜、后补齐”区分渐进式渲染、异步加载、流式渲染、BigPipe、懒加载与骨架屏。
- [[React组件化前端开发]]：理解组件、JSX、Props、State、Hooks、Render/Commit，以及 React 与浏览器、框架和服务端渲染的边界。
- [[Relay：React与GraphQL的数据管理|Relay]]：区分 React、GraphQL 与 Relay，理解组件数据需求、缓存一致性、数据修改和乐观更新。
...
- [[Cookie、Session与登录状态]]：先用“小纸条”和登录房卡认识 Cookie 是什么、存在哪里、有什么用，再学前端与 Spring Boot 设置、调试及安全边界。
- [[Spring、Spring Boot、Spring MVC与SSM：Java后端框架关系|Java 后端：Spring、Spring Boot、Spring MVC 与 SSM]]：用订单查询串起组件管理、Web 请求、数据库访问和构建工具，理解 Boot 与 MVC 可同时使用、SSM 是组合称呼。
- [[MyBatis：SQL映射与Java数据库访问|MyBatis]]：理解 SQL、JDBC、Mapper 和对象映射，区分它与数据库、ORM 及参数拼接，并关联 Spring 集成。
- [[Django]]：用 Python 开发网站后端的完整框架。
- [[Celery分布式任务队列]]：先区分“任务已受理”与“最终结果”、浏览器与服务器后台，再理解任务执行、结果查询或通知，以及队列、重试、幂等和监控。
...
- [[具身智能与离身智能：感知、行动与环境交互]]：区分信息处理与身体交互，理解传感器、执行器、闭环、VLA、机器人学习和边缘计算的关系。
- [[世界模型：环境预测、想象与规划]]：区分观察、状态、动作、策略与环境预测，理解训练数据、潜在状态、想象学习、规划和预测误差，以及与语言模型和视频生成的区别。
...
- [[LangChain]]：组织模型、工具、提示词和 Agent 的代码框架，并区分它与 Dify 应用平台、LangGraph 编排运行时的职责。
- [[Dify：可视化AI应用、知识库与工作流平台|Dify]]：用可视化方式连接模型、知识库与工具，理解自托管边界；含 [[Dify：可视化AI应用、知识库与工作流平台#Dify 与 LangChain 的详细区别|与 LangChain 的详细比较、选型及迁移]]。
- [[RAG、Naive RAG与GraphRAG]]：让大模型先查外部资料，再基于资料回答。
- [[LLM Wiki]]：让大模型把原始资料持续编译成结构化、互相链接、可长期维护的 Markdown 知识库。

## 05 AI Agent 与框架

- [[Dify：可视化AI应用、知识库与工作流平台#Dify 与 Agent 的区别|Dify 与 Agent 的区别]]：区分搭建平台、智能体和大模型，理解固定工作流与自主选择行动的差别。
- [[ReAct：推理、行动与观察的Agent模式|ReAct]]：区分前端 React、智能体 ReAct 与 Relay，理解推理、工具行动和真实观察怎样形成工作循环。
- [[Agent Harness：模型之外的运行系统|Agent Harness]]：拆开理解模型、工具和运行支撑，区分提示词、ReAct、MCP 与 Harness，并认识权限、进度和验收。
- [[LangGraph：有状态工作流与LangChain的区别|LangGraph 与 LangChain]]：用状态、节点和边组织任务；含 [[LangGraph：有状态工作流与LangChain的区别#Agent 的 Workflow 主要做什么|Agent 工作流的主要职责]]，解释流程控制、数据传递、人工审核、状态恢复和失败处理。
- [[Mastra：TypeScript智能体框架与LangChain、LangGraph对比|Mastra 与三框架对比]]：横向比较定位、语言、流程控制和开发代价，理解 LangChain 与 LangGraph 的关系，以及选型、学习路线、部署和迁移边界。
- [[LangChain、LangGraph与Mastra：真实产品与公开案例|三种框架的真实产品案例]]：核实 Flowise、Remote、LinkedIn、Klarna、Replit、Salesforce 与 Sanity 的具体使用范围，区分历史架构、产品功能和生成的应用。
...
- [[Control Plane、Data Plane、Edge Gateway与Federation]]：理解控制面、数据面、Otto 模型入口和跨部署协作，注意其中 Edge Gateway 不等于工业设备接入网关。
