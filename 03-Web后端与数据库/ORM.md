...
仍然需要理解表设计、主键、外键、[[Index的常见含义#二、数据库中的 Index：加速查询的辅助结构|索引]]、事务和 SQL。
...
3. `JOIN`；
4. 主键、外键、唯一约束；
5. [[Index的常见含义#二、数据库中的 Index：加速查询的辅助结构|索引]]；
6. 事务。

每写一个 ORM 查询，都尝试想一想它可能生成什么 SQL。

系统学习可从 [[SQL与关系型数据库学习地图]] 进入，再用 [[SQL订单实战：样例数据与练习]] 练习。复杂统计、CTE 和窗口见 [[SQL基础查询与多表关联]]、[[SQL窗口函数：排名、累计与同期比较]]；N+1 和生成 SQL 的性能检查见 [[SQL索引、执行计划与性能优化]]。对象映射不能替代 [[关系型数据库设计：建模、约束与范式]] 与 [[SQL事务、锁与并发一致性]]。
