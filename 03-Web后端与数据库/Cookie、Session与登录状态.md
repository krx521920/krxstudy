...
**Cookie 就是网站让浏览器保存的一小段信息，像一张“小纸条”，可以帮助网站记住登录状态、语言选择等事情。**

Cookie 读作“库基”，这个英文单词本来是“饼干”的意思，不是英文缩写。计算机里借用这个名字，表示一种由浏览器保存和按规则带回网站的数据，不是真的饼干。

> [!tip] 从零开始时怎么读
> 先看下面“不看代码”的解释。能说清楚“谁保存、带回给谁、有什么用”就可以先停下；后面的编程示例和安全属性留到需要开发时再学。

## 不看代码，先理解 Cookie 是什么

### 先分清浏览器和网站

- **浏览器**：你用来打开网页的软件，例如 Chrome、Edge。
- **网站**：你通过浏览器访问的服务，例如购物网站、课程网站。支撑网站的服务器，可以先理解为处理访问、保存网站资料的电脑。
- **Cookie**：由浏览器保存的一小段数据，位于你这边；网站那边还可能有与它对应的记录。

它不是一款要安装的软件，也不是一段会自行运行的程序。你通常不会在桌面看到一个“Cookie 图标”，因为这些数据由浏览器管理。

### 一个最简单的例子：记住你选择了中文

假设一个课程网站有中文、英文两种显示方式：

1. 你选择“中文”。
2. 网站让浏览器记下一条信息，大意是“这个浏览器的用户选择了中文”。
3. 以后访问这个网站时，浏览器在符合规则的情况下把这条信息带回去。
4. 网站读取它，就可以继续给你显示中文。

**这张用来记录语言选择的“小纸条”，就可以是一条 Cookie。** 网站也能用其他办法保存偏好，这里只是在解释 Cookie 的一种用途。

### 为什么登录后换个页面，不用重新输密码

登录 Cookie 可以类比酒店房卡：你办理入住后，酒店发房卡；之后不必每次进房间都重新登记身份。

网站常见的做法是：

1. 你输入账号和密码，网站检查正确。
2. 网站发给浏览器一个难以猜测的临时编号，并在自己这边记录它对应的登录状态；这个编号可以放在 Cookie 里。
3. 你再点“我的订单”时，浏览器带上编号；网站检查编号仍有效，就能继续识别这次登录。

**通常不是把密码写进 Cookie，而是让它携带一个临时凭证。** Cookie 不会自动赋予权限，还要由网站检查；它也不会天然告诉网站你的真实姓名。

### 所以它到底装的是什么

可以是很简单的“名称＋内容”，例如一条语言偏好，或者一串没有直观含义的编号。它不像整张网页那么大，也不是把你的账号资料全部复制一份。

“语言偏好”和“登录编号”可以分别保存在不同 Cookie 里，所以一个网站可能有多条 Cookie。浏览器按网站范围、有效期和安全规则管理它们，**不是你访问任何网站，都把全部 Cookie 交出去。**

### 清理 Cookie 会怎样

清理某个网站的 Cookie 后，它可能忘记你的语言选择，或要求重新登录。这不等于删除了网站里的账号、订单或课程，因为那些资料通常保存在网站那边。

Cookie 本身不等于病毒，但不同内容的敏感程度不同：语言偏好通常不敏感，登录 Cookie 则可能像临时钥匙一样重要。不要随意把完整登录 Cookie 发给别人。

先记住这一句即可：**网站发“小纸条”，浏览器替你保管，之后按规则带回对应的网站。** 基础机制参考 [MDN：Cookie 入门](https://developer.mozilla.org/en-US/docs/Web/HTTP/Guides/Cookies)。

## 理解概念以后，再看开发方式

开发时可以由后端用 `Set-Cookie` 设置，也可以由网页用 `document.cookie` 设置普通 Cookie。登录凭证应由后端在验证身份后，通过受保护的 Cookie 下发。下面开始讲实现，不需要在第一次认识这个词时全部掌握。
