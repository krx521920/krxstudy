...
模型不是直接在某张“问题—答案表”中查询，而是大致经历下面的过程：

1. 分词器把文字切成 Token，并转换成 Token ID。
2. Embedding 把每个 Token ID 变成向量，再加入位置信息。
3. 在某个注意力头中，“是”这个位置产生自己的 Query，表达当前需要从上下文中寻找什么信息。
4. 它用 Query 与“法国”“的”“首都”“是”等位置的 Key 计算匹配分数。
5. Softmax 把分数变成权重；在某些层和注意力头中，“法国”和“首都”可能获得较高权重。
6. 模型按权重汇总这些位置的 Value，使“是”位置的新向量带上“法国首都”这一上下文关系。
7. 多个注意力头、前馈网络和多层 Transformer 继续加工这个向量。
8. 最后的输出层把向量变成词表中每个候选 Token 的分数，模型可能给“巴黎”较高概率。

简化示意：

```mermaid
flowchart LR
    Input["法国 / 的 / 首都 / 是"] --> Vector[Token 向量]
    Vector --> Score[Q 与各位置 K 计算分数]
    Score --> Weight[Softmax 得到注意力权重]
    Weight --> Mix[加权汇总各位置 V]
    Mix --> Layers[经过多头、多层和前馈网络]
    Layers --> Logits[得到下一个 Token 的分数]
    Logits --> Paris[选择“巴黎”]
```

这里要注意三点：

- 注意力负责**在当前上下文内部交换和汇总信息**，不是自动联网搜索。
- “巴黎”相关知识主要来自训练形成的模型参数；注意力帮助模型结合当前问题调用内部表示。
- 某个注意力权重较高，不等于它单独决定了答案。最终结果由多层、多头、前馈网络和输出层共同形成。
...
- [[大模型从数据、Token到训练与本地部署]]：从原始资料、Tokenizer、Token ID、预训练到下载权重进行本地推理的完整链路。
