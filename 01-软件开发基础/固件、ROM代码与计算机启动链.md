...
- [[计算机硬件与底层软件学习地图]]：把固件放回电路板、处理器、存储、驱动与操作系统的完整关系中。
- [[存储层次：RAM、SRAM、DRAM、Cache与Flash]]：区分固件代码、ROM 类别和 Flash 存储介质，理解断电保存与运行时内存。
- [[操作系统、内核与硬件管理]]：理解固件启动系统后，内核怎样接管资源管理，以及运行时固件为什么仍可能参与。
